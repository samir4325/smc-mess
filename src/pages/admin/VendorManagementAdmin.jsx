...
const schema = z.object({
  name:            z.string().min(2, 'Shop/Vendor name required'),
...
export function VendorManagementAdmin() {
  ...
  const onSubmit = (data) => {
    if (editVendor) {
      updateVendor(editVendor.id, data, currentUser)
      toast.success('Vendor updated!')
      setEditVendor(null)
    } else {
      addVendor(data, currentUser)
  ...
  const FormBody = () => (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      <div><label className="form-label">Contact Person *</label><input {...register('contactPerson')} className="form-input" />{errors.contactPerson && <p className="form-error">{errors.contactPerson.message}</p>}</div>
      <div><label className="form-label">Mobile *</label><input {...register('mobile')} className="form-input" type="tel" />{errors.mobile && <p className="form-error">{errors.mobile.message}</p>}</div>
      <div><label className="form-label">Alternate Mobile</label><input {...register('alternateMobile')} className="form-input" type="tel" /></div>
      <div className="sm:col-span-2"><label className="form-label">Address *</label><textarea {...register('address')} className="form-input" rows={2} />{errors.address && <p className="form-error">{errors.address.message}</p>}</div>
      <div><label className="form-label">Email</label><input {...register('email')} className="form-input" type="email" /></div>
  ...
  return (
    <div className="space-y-5">
      ...
      <Modal isOpen={addOpen} onClose={() => setAddOpen(false)} title="Add Vendor" size="lg"
        footer={<><button className="btn-secondary" onClick={() => setAddOpen(false)}>Cancel</button><button className="btn-primary" onClick={handleSubmit(onSubmit)}>Add Vendor</button></>}>
        <FormBody />
      </Modal>
      <Modal isOpen={!!editVendor} onClose={() => setEditVendor(null)} title="Edit Vendor" size="lg"
        footer={<><button className="btn-secondary" onClick={() => setEditVendor(null)}>Cancel</button><button className="btn-primary" onClick={handleSubmit(onSubmit)}>Save</button></>}>
        <FormBody />
      </Modal>
    </div>
  )
